...
public class plot_summary_parameters {
	...
	public plot_summary_parameters(double [] coeff,int[] marks,int[] image,int[]totalWord,float[]submissionTime){
		actualMarks = marks;
		imageCount = image;
		...
		scatter_plot sp = new scatter_plot("Acutal Mark v/s Predicted Mark","Predicted Mark","Actual Mark",yData,xData);
		sp.pack();
		sp.setVisible(true);
		}else{
	...
	private void createYData(){
		...
		for(int i=0;i<yData.length;i++){
			yData[i]  = summaryCoeff[0]+(summaryCoeff[1]*totalWordCount[i])+(summaryCoeff[2]*imageCount[i])+(summaryCoeff[3]*submissionTimeLeft[i]);
